...
📌 الوصف:
    ...
    على منصة NoteGPT عبر Pure Requests بدون الحاجة لفتح المتصفح أو إنشاء أي حسابات.

✨ المميزات:
    • 📂 مجلد الإسقاط السريع الذكي (Auto Drop-Folder `chat_attachments/`):
        - اسحب أي صورة أو ملف بالماوس إلى المجلد والسكربت يلقطه تلقائياً فوراً!
        - دعم فحص الصور وقراءة أبعادها وبياناتها الوصفية تلقائياً.
        - دعم ملفات الأكواد والبيانات والمستندات (Python, JSON, Markdown, CSV, Text).
    • 📎 استيعاب الروابط الخارجية (YouTube Transcripts & Webpages).
    • 🔄 تدوير الـ IP التلقائي مع كل سؤال + تدوير كامل للهوية عند نفاد الرصيد.
    • 💬 البقاء دائماً في نفس الشات ومواصلة السياق بضغطة زر Run.
...
class SourceIngestionHandler:
    """
    محلل المرفقات الذكي:
    1. يفحص مجلد chat_attachments/ تلقائياً ويسحب أي صورة أو ملف جواه بدون أي كلمات في النص.
    2. يفحص الصور ويستخرج أبعادها ومعلوماتها بدقة.
    ...
    def build_injected_prompt(cls, sources: List[SourceItem], user_prompt: str) -> str:
        ...
        for idx, src in enumerate(sources, 1):
            if src.type == "image":
                ...
                blocks.append("💡 يرجى تفريغ وتحليل محتوى الفيديو واستخدامه في الساندبوكس.\n")
...
def print_banner(client: 'NoteGPTAgentClient', sources: List[SourceItem] = None):
    ...
    print(f"{C_CYAN}📌 مجلد الإسقاط السريع chat_attachments/ + 👁️ التعرف على الصور والأكواد{C_RESET}")
    print_divider("─", C_MAGENTA)
...
class NoteGPTAgentClient:
    """كلاينت إدارة وتنفيذ طلبات الأيجنتس الحقيقية والـ Sandbox مع التدوير التلقائي للـ IP والهوية"""
    ...
    def _build_headers(self) -> Dict[str, str]:
        """بناء الهيدرات وتجديد عنوان الـ IP تلقائياً مع كل سؤال جديد"""
        self.current_ip = generate_fake_ip()
        return {
            'accept': "*/*",
    ...
    def ask_agent_stream(self, prompt: str) -> Generator[Dict[str, Any], None, None]:
        """
        🚀 إرسال طلب الأيجنت واستقبال البث الحي مع تدوير الـ IP والتعافي الذاتي في نفس الشات
        """
...
def print_execution_report(client: 'NoteGPTAgentClient', prompt: str, elapsed: float, total_credits: int, saved_files: list, sources: List[SourceItem] = None):
    ...
    if sources:
        print(f"  📂 المرفقات المعتمدة: {C_YELLOW}{len(sources)}/{Config.MAX_ATTACHED_SOURCES} مرفقات من مجلد chat_attachments{C_RESET}")
    print_divider("─", border_color)
...
def run_single_prompt(client: NoteGPTAgentClient, raw_prompt: str, base_dir: pathlib.Path = None) -> str:
    """تنفيذ مهمة واحدة مع مسح مجلد chat_attachments/ وحقن المرفقات تلقائياً"""
    if base_dir is None:
    ...
    if sources:
        print(f"{C_MAGENTA}📂 [مجلد المرفقات]: تم التقاط {len(sources)} مرفق من مجلد chat_attachments بنجاح.{C_RESET}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="🚀 سكربت تشغيل وضع الأيجنتس الحقيقي والـ Sandbox (NoteGPT Real Agent Engine v01.04)",
