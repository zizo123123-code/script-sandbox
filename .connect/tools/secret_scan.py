...
#
# `\{[^{}]*\}` = موضع استبدال في f-string أو `.format()`. الفحص ساكن يقرأ
# **النص المصدري**، فقيمة كـ `f'PASSWORD = "{pw}"'` لا تحتوي سراً في المصدر
# إطلاقاً — السرّ (لو وُجد) يكون في المتغير، وتلك القواعد الأخرى تلتقطه عند
# تعريفه. وهو نفس منطق `<[^>]*>` الموجود أصلاً: علامة استبدال لا قيمة.
...
# اسم متغير بيئة — ليس قيمة سرّ.
#
# الجذر: القواعد تلتقط `<مفتاح حسّاس> = "<قيمة>"` بلا تمييز بين أن تكون القيمة
# سرّاً فعلياً أو **اسم** متغير البيئة الذي سيُقرأ منه السرّ:
#     ENV_PASSWORD = "NOTEGPT_PASSWORD"   ← اسم، وهو الآلية الصحيحة
#     PASSWORD     = "Password123#$"      ← سرّ حقيقي
# القياس المُثبَت: `config.py:105-106` رُفِعا كسرّين، وهما تحديداً الثوابت التي
# تُبعد الأسرار عن الكود — أي أن الفحص كان يُعاقب النمط الصحيح. وهذا هو نفس
# جذر عيب حاجز الحزمة (تعذُّر التمييز بين الاسم والقيمة)، من الطرف المقابل.
#
# المعيار بنيوي لا موضعي: القيمة نفسها معرِّف SCREAMING_SNAKE بالكامل. كلمة مرور
# حقيقية يجب أن تكون حروفاً كبيرة وأرقاماً وشُرَطاً سفلية فقط لتتسلل من هنا،
# وهو ما تستبعده أي سياسة كلمات مرور. لا يُعتمد على اسم الملف إطلاقاً.
ENV_VAR_NAME_VALUE = re.compile(r"^[A-Z][A-Z0-9_]*$")
...
def is_safe(value: str, line: str, rule_id: str = "") -> bool:
    ...
    if TEST_VALUE_MARKERS.search(value.strip()):
        return True
    # اسم متغير بيئة (لا قيمته). يُطبَّق حتى على القواعد الحسّاسة للاتجاه، لأن
    # `os.environ["X"] = "SOME_NAME"` تمرير اسم لا حقن سرّ.
    if ENV_VAR_NAME_VALUE.match(value.strip()):
        return True
